...
export const Radar = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-40 h-[1px] w-full bg-gradient-to-r from-transparent via-[var(--cyan)] to-transparent opacity-80" />
      </div>
      {/* Concentric circles */}
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(0, 212, 255, ${Math.max(0.05, 0.4 - (idx + 1) * 0.05)})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
