...
// Tetris pieces - pure black and white (Enfibio theme adapted: blue/slate)
const TETRIS_PIECES = [
  { shape: [[1, 1, 1, 1]], color: 'bg-blue-500' },
  { shape: [[1, 1], [1, 1]], color: 'bg-emerald-500' },
  { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-indigo-500' },
  { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-teal-500' },
  { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-sky-500' },
  { shape: [[1, 1, 0], [0, 1, 1]], color: 'bg-cyan-500' },
  { shape: [[0, 1], [0, 1], [1, 1]], color: 'bg-blue-600' },
]
...
export default function TetrisLoading({ 
  ...
  const sizeConfig = {
    ...
    md: { cellSize: 'w-4 h-4', gridWidth: 10, gridHeight: 20, padding: 'p-1' },
    lg: { cellSize: 'w-6 h-6', gridWidth: 10, gridHeight: 20, padding: 'p-1.5' }
  }
        ...
        {row.map((cell, colIndex) => (
          <div
            ...
            className={`${config.cellSize} border border-white/10 transition-all duration-100 ${
              cell.filled ? `${cell.color} scale-100 shadow-[0_0_10px_rgba(255,255,255,0.2)]` : 'bg-[var(--surface)] scale-95'
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <div className="mb-8">
        <div className={`border border-white/20 bg-[var(--surface-2)] shadow-2xl ${config.padding} rounded-md overflow-hidden`}>
          {renderGrid()}
        </div>
      </div>
      {showLoadingText && (
        <div className="text-center font-mono text-sm tracking-widest text-blue-500 animate-pulse">
          {loadingText}
        </div>
      )}
    </div>
